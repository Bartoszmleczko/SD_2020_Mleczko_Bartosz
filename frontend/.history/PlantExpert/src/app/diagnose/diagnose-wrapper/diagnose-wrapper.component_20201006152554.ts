...
export class DiagnoseWrapperComponent implements OnInit {
  ...
  getDiagnoseFormData(templateName: string) {
    ...
    this.diagnoseService.getDiagnoseForm(templateName).pipe(
        map(obj => {
          this.formData.riskFactors = obj.riskFactors;
          this.formData.symptoms = obj.symptoms;
        })
      );

    console.log(this.formData.riskFactors);
